...
This creates `resources/entities/products.edn`, migration files, and `src/pizza/hooks/products.clj`. Refresh the browser and the entity appears in the menu.
...
```bash
lein uberjar
java -jar target/uberjar/pizza-0.1.0-standalone.jar
```
...
{:connections
 {:sqlite   {:db-type  "sqlite"
             ...
             :db-name  "db/pizza.sqlite"}

  :mysql    {:db-type  "mysql"
             :db-class "com.mysql.cj.jdbc.Driver"
             :db-name  "//localhost:3306/pizza"
             :db-user  "root"
             :db-pwd   "password"}

  :postgres {:db-type  "postgresql"
             :db-class "org.postgresql.Driver"
             :db-name  "//localhost:5432/pizza"
             :db-user  "postgres"
             :db-pwd   "password"}
...
```clojure
(require '[pizza.engine.config :as config])
(config/reload-all!)
```
