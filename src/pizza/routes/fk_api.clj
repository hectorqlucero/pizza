(ns pizza.routes.fk-api
  (:require
   [compojure.core :refer [defroutes GET POST]]
   [pizza.engine.config :as config]
   [pizza.engine.crud :as crud]
   [pizza.models.crud :as model-crud]
   [pizza.models.util :refer [json-response]]
   [pizza.i18n.core :as i18n]
   [clojure.string :as str]
   [clojure.data.json :as json]
   [pizza.engine.render :as render]
      ...
      (try
        (let [entity-config (config/get-entity-config entity)
              ...
              form-fields (map #(select-keys % [:id :label :type :required? :placeholder
                               ...
                               fields)
              rendered (let [render-fn #'pizza.engine.render/render-field]
                         (->> fields
                              (map #(render-fn % {}))
                              (html)))]
